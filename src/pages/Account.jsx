                        ...
                        <div className={cn("p-6 rounded-2xl border transition-all relative flex flex-col justify-between", subscription === 'free' ? "border-primary ring-1 ring-primary/20 bg-primary/5" : "border-border bg-card shadow-sm")}>
                            ...
                            <div>
                                {subscription === 'free' ? (
                                    ...
                                    <button
                                        onClick={() => {
                                            if (confirm("Switch to Free plan? You will be limited to 50 txns/mo and 1 member max.")) {
...
const HouseholdCard = () => {
    ...
    const activeCode = inviteCode || householdMeta?.inviteCode;
    const isAtSeatLimit = householdMemberBalances.length >= seatLimit;
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm md:col-span-2">
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-muted border border-border">
                    {householdMemberBalances.length} / {seatLimit} members
                </span>
            </div>
            ...
            {/* Free Plan seat limit notification */}
            {subscription === 'free' && (
                <div className="mb-4 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 space-y-2">
                    <div className="flex items-center gap-2 font-bold text-sm">
                        <Crown className="w-4 h-4" /> Free Plan Household Limit (1 Member Max)
                    </div>
                    <p className="text-xs text-foreground/80 leading-relaxed">
                        Your account is currently on the <strong>Free Plan</strong>, which supports <strong>1 member (owner only)</strong>. To invite family or team members to your household, please upgrade your subscription:
                    </p>
                    <div className="flex flex-wrap items-center gap-2 pt-1">
                        <button
                            ...
                            className="px-3.5 py-1.5 rounded-lg bg-amber-500 text-white font-bold text-xs shadow-sm hover:opacity-90 transition-all flex items-center gap-1.5"
                        >
                            <Crown className="w-3.5 h-3.5" /> Upgrade Plan (Starter ₹9 / Pro ₹100)
                        </button>
                    </div>
                </div>
            )}

            {/* Paid Plan seat limit reached banner */}
            {subscription !== 'free' && isAtSeatLimit && (
                <div className="mb-4 p-3.5 rounded-xl bg-muted border border-border text-xs space-y-1">
                    <div className="flex items-center justify-between font-bold text-foreground">
                        <span>Seat Limit Reached ({householdMemberBalances.length} / {seatLimit} members)</span>
                        {seatLimit < 4 && (
                            <button
                                type="button"
                                onClick={() => {
                            ...
                            >
                                Upgrade to Pro (4 seats)
                            </button>
                        )}
                    </div>
                    <p className="text-muted-foreground">
                        {seatLimit < 4
                            ? "Upgrade to Pro Plan to invite up to 4 family or team members!"
                            : "Your household has reached the maximum size of 4 members."}
                    </p>
                </div>
            )}
                ...
                <div className="mb-4 p-4 rounded-xl bg-primary/10 border border-primary/20 space-y-3">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm font-bold text-primary">
                            ...
                            <span>Invite Teammate / Family Member</span>
                        </div>
                        <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-primary/20 text-primary">
                            {seatLimit - householdMemberBalances.length} seat(s) available
                        </span>
                    </div>
            ...
            <div className="space-y-2.5 mb-4">
                {householdMemberBalances.map((m) => (
                    <div key={m.uid} className="flex items-center justify-between p-3 rounded-xl border border-border">
                        ...
                        {isOwner && !m.isMe && (
                            <button onClick={() => handleRemove(m.uid)} disabled={loading} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-lg flex-shrink-0" title="Remove member">
                                <Trash2 className="w-4 h-4" />
                            </button>
                        )}
                    </div>
            ...
            <div className="flex gap-3">
                {isOwner && (
                    <button onClick={handleGetInviteCode} disabled={loading} className="flex-1 py-2.5 rounded-xl border border-border text-sm font-semibold flex items-center justify-center gap-2">
                        <UserPlus className="w-4 h-4" /> Regenerate Invite Code
                    </button>
                )}
